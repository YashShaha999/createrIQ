    ...
    const checkDb = async () => {
      try {
        ...
        if (res.data?.status === 'healthy') {
          setDbStatus({ healthy: true, text: 'MongoDB Atlas ✅' });
        } else {
          setDbStatus({ healthy: false, text: 'MongoDB Offline' });
        }
      } catch (err) {
      ...
      <aside className={`app-sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <Link to="/dashboard" className="sidebar-brand" onClick={() => setSidebarOpen(false)}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Analytics & Management</div>
          </div>
        </Link>
        ...
        <ul className="sidebar-nav">
          ...
          {user?.role === 'admin' && (
            <>
              <li className="nav-section-title" style={{ marginTop: '0.75rem' }}>Management</li>
              <li>
                <Link 
                  to="/admin" 
                  ...
                  onClick={() => setSidebarOpen(false)}
                >
                  <span className="nav-item-icon"><ShieldCheck size={18} color="#10b981" /></span>
        ...
        <header className="top-header">
          <div className="header-left">
            ...
            <div className="header-title-wrap">
              <h1 className="header-page-title">{pageTitle || 'CreatorIQ Platform'}</h1>
            </div>
          </div>

          <div className="header-right">
            <div className="db-status-pill">
              <span className="status-dot" style={{ background: dbStatus.healthy ? '#10b981' : '#f87171' }} />
